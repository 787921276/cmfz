...
public class ChapterController {
    ...
    public Map<String, Object> findByPage(Integer page, Integer rows, String albumid) {
        Integer count = chapteService.count(albumid);

        Integer total = count % rows == 0 ? count / rows : count / rows + 1;

        List<chapter> byPage = chapteService.findByPage(page, rows, albumid);
    ...
    public String edit(chapter chapter, String oper, String albumid, String[] id) {
        if ("add".equals(oper)) {
            String s = chapteService.save(chapter);
            return s;
        }
        if (oper.equals("del")) {
            chapteService.delete(id);
